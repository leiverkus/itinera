...
class MultiCriteriaFrictionAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return ("Combines several penalty rasters (hydrology, wetness, land "
                ...
                "surface (Herzog 2022; Litvine et al. 2024). Each layer is "
                "min-max normalised to 0–1, optionally inverted (so a high input "
                "value becomes a low cost), weighted, and combined by a weighted "
                "sum (linear) or product (geometric) into a friction in the "
                "chosen range.\n\n"
