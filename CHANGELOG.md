...
- **Movement-mode cost presets** — two anisotropic critical-slope cost functions,
  `wheeled` (cart, ~8 % critical upward slope) and `pack_animal` (~25 %), where
  cost rises quadratically with grade and the uphill limit is tighter than
  downhill (Herzog 2013; Verhagen et al. 2019). They appear in every
  cost-function dropdown — ten cost functions total.
- **Accessibility / cost catchment** — a Processing algorithm (group "Cost
  surfaces") and `core/accessibility.py::accessibility` that compute the
  cost-distance (movement-potential) surface from source point(s), plus an
  optional catchment mask (the area reachable within a cost budget) and isochrone
  bands. Reuses the deterministic Dijkstra accumulation.
...
- **DEM error realisation** — a standalone Processing algorithm (group "Tools")
  that outputs one perturbed DEM, and `core/stochastic.py::simulate_error_field`
  in the library.

### Changed

- **Variogram-based DEM error fields.** The stochastic DEM-error model is now a
  true geostatistical Gaussian random field — exponential (new default),
  spherical or gaussian variogram with an optional nugget — generated by FFT
  spectral simulation (`scipy.fft`, no `gstat` dependency; Hunter & Goodchild
  1997), replacing the previous Gaussian-filter approximation. The fast Gaussian
  filter is retained as a selectable model. The *Stochastic least-cost path*
  algorithm gains **DEM error model** and **nugget** parameters; `add_dem_error`
