...
field. The default is a **variogram** model (exponential / spherical / gaussian,
with an optional nugget), generated by FFT spectral simulation — no
`gstat`/geostatistics dependency, just `scipy.fft`. A fast `"gaussian_filter"`
approximation (white noise smoothed with a Gaussian kernel) is kept as an option.

Provenance: this is an **Itinera implementation in the spirit of** the
spatially-autocorrelated DEM-error literature, **not a line-for-line reproduction
of any one method**. Hunter & Goodchild (1997) use an autoregressive (AR) spatial
error model; Lewis (2021)'s stochastic LCP uses filtered Gaussian noise followed
by a sink-fill step. Itinera instead simulates a chosen variogram covariance by
FFT spectral synthesis (circulant embedding). All three produce correlated
Gaussian DEM error, but the generators differ — cite accordingly.
"""
...
def simulate_error_field(shape, rmse, vrange, cellsize, rng,
    ...
    simulation (generator: Dietrich & Newsam circulant embedding; in the spirit
    of the correlated DEM-error literature, not a reproduction of a specific
    published model — see the module docstring).
...
def _max_ci_error(freq, k, z=_WILSON_Z):
    ...
    to bring the **per-cell (pointwise) 95% interval** of every cell — including
    low-probability ones — below ``tol``. Caveats on the strength of this
    guarantee: (1) the intervals are **pointwise, not simultaneous** — they bound
    each cell at 95% individually, *not* all cells jointly at 95% (with many
    cells, some will exceed their interval by chance; a family-wise bound would
    need a multiple-comparison/Bonferroni-style correction). (2) It is a per-check
    interval, not a formal confidence *sequence*: with many interim checks the
    effective per-cell coverage is also looser than the nominal 95%. So this
    reduces — does not eliminate — the chance of stopping on a rare-route
    artefact. Lower ``tol`` or raise ``check_every`` if either matters.
...
def stochastic_lcp(dem, cellsize, cost_fn, origin, destinations, n_iter, rng,
    ...
    * ``convergence="precision"`` — stop once the **maximum per-cell (pointwise)
      Wilson 95% confidence-interval error** over all cells is below ``tol``: for
      each cell the larger gap from the reported fraction ``p_hat`` to either
      interval bound, ``max(p_hat - lower, upper - p_hat)``. This bounds each
      cell's route probability to ``±tol`` at 95% *pointwise* — not a
      simultaneous guarantee across all cells (see ``_max_ci_error``).
